async function sendViaResend({ to, subject, html }) {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from: `Anidev <${from}>`, to, subject, html }),
  });
...
function buildOrderHtml({ transactionId, orderId, customer, items, total, paymentMethod, isAdmin }) {
  ...
  const title = isAdmin ? 'New Grocery Order' : 'Order Confirmation — Anidev';
  const greeting = isAdmin
    ? `<p>A new order has been placed on Anidev.</p>`
    : `<p>Dear ${customer.name},</p><p>Thank you for ordering from Anidev! ${paymentMethod === 'cod' ? 'Please keep cash ready for delivery.' : 'Your payment was successful.'}</p>`;

  return `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
      <h2 style="color:#00897b;">${title}</h2>
      ${greeting}
...
async function sendOrderEmails({ transactionId, orderId, customer, items, total, paymentMethod = 'online' }) {
  const adminEmail = process.env.ADMIN_EMAIL;
  const buyerHtml = buildOrderHtml({ transactionId, orderId, customer, items, total, paymentMethod, isAdmin: false });
  const adminHtml = buildOrderHtml({ transactionId, orderId, customer, items, total, paymentMethod, isAdmin: true });

  await sendViaResend({
    to: customer.email,
    subject: `Anidev Order Confirmed — ${transactionId}`,
    html: buyerHtml,
  });
  ...
  if (adminEmail) {
    await sendViaResend({
      to: adminEmail,
      subject: `[Anidev] New Order — ${transactionId}`,
      html: adminHtml,
    });
  }
}
...
async function sendContactEmail({ name, email, phone, message }) {
  ...
  await sendViaResend({
    to: adminEmail,
    subject: `[ANIDEV Contact] Message from ${name}`,
